...
export function initializeDiscordRoleMonitor(): void {
  ...
  const shouldStart =
    process.env.NODE_ENV === 'production' || process.env.DISCORD_ROLE_MONITOR_ENABLED === 'true';

  if (!shouldStart) {
    logger.info(
      'Discord role monitor not starting (not in production and not explicitly enabled)',
      {
  ...
  if (!discordToken || !discordGuildId) {
    logger.warn(
      'Discord role monitor not starting: DISCORD_BOT_TOKEN and DISCORD_GUILD_ID environment variables required',
      {
        module: 'discord-monitor-init',
      }
    );
  ...
  try {
    ...
    logger.error(
      'Failed to initialize Discord role monitor',
...
export async function cleanupDiscordRoleMonitor(): Promise<void> {
  try {
    const monitor = getDiscordRoleMonitor();
    await monitor.stop();
    ...
    logger.error(
      'Error stopping Discord role monitor',
      error instanceof Error ? error : undefined,
      {
        module: 'discord-monitor-init',
      }
