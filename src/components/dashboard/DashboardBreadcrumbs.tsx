...
export default function DashboardBreadcrumbs() {
  ...
  const breadcrumbs = segments.map((segment, index) => {
    ...
    // Decode any URL-encoded characters before formatting
    let raw = segment;
    try {
      raw = decodeURIComponent(segment);
    } catch {
      // Leave raw as-is if the segment is not valid URI-encoded text
    }
      ...
      {breadcrumbs.map((crumb, index) => {
        ...
        return (
          <div key={crumb.href} className="flex items-center">
            {index > 0 && <span className="mx-2 text-[rgba(var(--mg-primary),0.4)]">/</span>}
