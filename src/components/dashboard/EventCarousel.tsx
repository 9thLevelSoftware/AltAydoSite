...
const CornerAccent = ({ position }: { position: 'tl' | 'tr' | 'bl' | 'br' }) => {
  ...
  return (
    <div
      className={`absolute w-5 h-5 ${classes[position]} border-[rgba(var(--mg-primary),0.6)]`}
    ></div>
  );
};
  ...
  const triggerFlashEffect = useCallback(() => {
    containerControls.start({
      boxShadow: [
        ...
        '0 0 15px rgba(var(--mg-primary), 0.3)',
        '0 0 0px rgba(var(--mg-primary), 0)',
      ],
    ...
    >
      ...
      <div className="relative p-4 flex items-center justify-between z-20 border-b border-[rgba(var(--mg-primary),0.2)]">
        <div className="flex items-center">
          ...
          <h2 className="mg-title text-sm font-quantify tracking-wider text-[rgba(var(--mg-primary),0.9)]">
        ...
        <div className="hidden lg:block text-xs text-[rgba(var(--mg-text),0.6)]">
          <span className="font-mono">
            FEED {String(currentIndex + 1).padStart(2, '0')}/
            ...
            <div className="relative h-full w-full">
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-[rgba(var(--mg-panel-dark),0.9)] to-transparent p-4">
                <div className="relative">
                  ...
                  <h3 className="font-quantify text-lg tracking-wide text-[rgba(var(--mg-primary),0.9)] mb-1">
          ...
          {images.map((_, index) => (
            <motion.button
              ...
              className={`relative h-1.5 rounded-full transition-all ${
                index === currentIndex
                  ? 'w-8 bg-[rgba(var(--mg-primary),0.9)]'
                  : 'w-3 bg-[rgba(var(--mg-text),0.3)]'
              }`}
              onClick={() => goToSlide(index)}
              whileHover={
                index !== currentIndex
