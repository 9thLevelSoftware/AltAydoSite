...
const SystemStatusPanel = () => {
  ...
  const StatusIcon = ({ status }: { status: SystemStat['status'] }) => {
    const colorClass = {
      optimal: 'bg-[rgba(var(--mg-success),0.9)]',
      warning: 'bg-[rgba(var(--mg-warning),0.9)]',
      critical: 'bg-[rgba(var(--mg-danger),0.9)]',
      inactive: 'bg-[rgba(var(--mg-text),0.4)]',
    };

    return <div className={`w-2 h-2 rounded-full ${colorClass[status]} animate-pulse`}></div>;
  };
  ...
  // NOTE: systemStats above are static sample values, not real telemetry.
  // Labeled SIMULATED so the UI does not imply live data until a telemetry
  // source is wired up.
  const headerRight = (
    <div className="text-xs text-[rgba(var(--mg-text),0.6)] flex items-center">
      <span className="mg-flicker">SIMULATED</span>
      <div className="w-1.5 h-1.5 rounded-full bg-[rgba(var(--mg-warning),0.8)] ml-2 animate-pulse"></div>
    </div>
  );
        ...
        <div>
          {systemStats.map((stat, index) => (
            ...
            >
              <div className="flex items-center">
                ...
                <div className="flex items-center ml-3">
                  {stat.icon && (
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="h-4 w-4 mr-2 text-[rgba(var(--mg-text),0.7)]"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
              ...
              <div
                className={`text-sm font-quantify text-[rgba(var(--mg-${getStatusColor(stat.status)}),0.9)]`}
              >
                {stat.value}
              </div>
            </motion.div>
