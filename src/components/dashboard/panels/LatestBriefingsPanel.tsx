...
// NOTE: Static demonstration data. There is no briefings data source yet
// (no Cosmos/local-JSON backing store or API route). Replace this constant
// with a real fetch once a briefings backend exists, and update the header
// "DEMO DATA" indicator and unread count accordingly.
const SAMPLE_BRIEFINGS: Briefing[] = [
  {
    id: 'brief-001',
    title: 'SECURITY PROTOCOL UPDATE',
    content:
      'All employees are required to update their security credentials by the end of the month. This includes biometric rescans and new authentication keys for vessel access.',
  ...
  },
  {
    ...
    content:
      'Our Q3 projections show a 15% increase in logistics revenue. Mining operations continue to exceed expectations with the implementation of new extraction technologies.',
    author: 'CFO Laurens',
  ...
  },
  {
    ...
    content:
      'We have established new trade routes through the Aaron Halo, increasing efficiency by 22%. All captains should download the latest navigation data.',
...
const LatestBriefingsPanel = () => {
  const briefings = SAMPLE_BRIEFINGS;
  const unreadCount = briefings.filter((b) => b.isUnread).length;
  ...
  const headerRight = (
    <div className="text-xs text-[rgba(var(--mg-text),0.6)] flex items-center gap-2">
      <span
        className="px-1.5 py-0.5 rounded-sm border border-[rgba(var(--mg-warning),0.4)] text-[rgba(var(--mg-warning),0.9)] text-[10px] tracking-wide font-quantify"
        title="Static demonstration data — not yet backed by a live data source"
      >
        DEMO DATA
      </span>
      {unreadCount > 0 && (
        <span className="flex items-center">
          <span className="w-1.5 h-1.5 rounded-full bg-[rgba(var(--mg-warning),0.8)] mr-1"></span>
        ...
        {briefings.map((briefing, index) => (
          ...
          >
            ...
            <div className="flex items-center px-3 py-2 border-b border-[rgba(var(--mg-primary),0.1)]">
              <div className="mr-2">
                <div
                  className={`w-5 h-5 flex items-center justify-center rounded-full bg-[rgba(var(--mg-${getCategoryColor(briefing.category)}),0.1)] border border-[rgba(var(--mg-${getCategoryColor(briefing.category)}),0.3)]`}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className={`h-3 w-3 text-[rgba(var(--mg-${getCategoryColor(briefing.category)}),0.9)]`}
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                    <path
                      ...
                      strokeLinejoin="round"
                      strokeWidth={1.5}
                      d={getCategoryIcon(briefing.category)}
                    />
                  </svg>
                </div>
              </div>
              <div className="flex-grow">
                <div className="font-quantify text-sm text-[rgba(var(--mg-primary),0.9)]">
                  {briefing.title}
                </div>
