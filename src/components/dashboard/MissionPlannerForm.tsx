...
const MissionPlannerForm: React.FC<MissionPlannerFormProps> = ({
  ...
  const fetchLeaders = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setLeadersError('Failed to load leaders. Please try again.');
      }
    } catch (error) {
  ...
  const totalShips = useMemo(() => {
    return (formData.shipRequirements || []).reduce(
  ...
  const totalPersonnel = useMemo(() => {
    return (formData.personnelRequirements || []).reduce(
      (total, requirement) => total + requirement.count,
      0
    );
  ...
  const removeShipRequirement = (index: number) => {
    onInputChange(
      'shipRequirements',
      formData.shipRequirements?.filter((_, i) => i !== index) || []
    );
  };
  ...
  const removePersonnelRequirement = (index: number) => {
    onInputChange(
      ...
      formData.personnelRequirements?.filter((_, i) => i !== index) || []
    );
  };
    ...
    <div className="space-y-6">
      ...
      <MobiGlasPanel title="Mission Details">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="md:col-span-2">
            <label htmlFor="mission-name" className="mg-subtitle block mb-2">
              MISSION NAME *
            </label>
            <input
              type="text"
          ...
          <div>
            <label htmlFor="mission-datetime" className="mg-subtitle block mb-2">
              SCHEDULED DATE & TIME *
            </label>
            <input
              type="datetime-local"
              id="mission-datetime"
              value={
                formData.scheduledDateTime ? formatDateTimeForInput(formData.scheduledDateTime) : ''
              }
              aria-required={true}
              onChange={(e) => {
                if (e.target.value) {
          ...
          <div>
            <label htmlFor="mission-duration" className="mg-subtitle block mb-2">
              ESTIMATED DURATION
            </label>
            <select
              id="mission-duration"
              ...
              onChange={(e) =>
                onInputChange('duration', e.target.value ? parseInt(e.target.value, 10) : undefined)
              }
          ...
          <div className="md:col-span-2">
            <label htmlFor="mission-location" className="mg-subtitle block mb-2">
              LOCATION / SYSTEM
            </label>
            <select
              id="mission-location"
      ...
      <MobiGlasPanel title="Operation Type">
              ...
              <div
                className={
                  formData.operationType === type
                    ...
                    : 'text-[rgba(var(--mg-text),0.6)]'
                }
              >
                {type === 'Space Operations' ? <SpaceIcon /> : <GroundIcon />}
              </div>
              <span
                className={`font-medium ${formData.operationType === type ? 'text-[rgba(var(--mg-primary),1)]' : 'text-[rgba(var(--mg-text),0.8)]'}`}
              >
                {type}
              </span>
            </motion.button>
        ...
        {errors.operationType && (
          <div className="text-[rgba(var(--mg-danger),0.8)] text-sm mt-2">
            {errors.operationType}
          </div>
        )}
      </MobiGlasPanel>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {ACTIVITIES.map((activity) => {
                const isDisabled =
                  activity === formData.secondaryActivity || activity === formData.tertiaryActivity;
                return (
                  <motion.button
                    key={activity}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {ACTIVITIES.filter(
                (a) => a !== formData.primaryActivity && a !== formData.tertiaryActivity
              ).map((activity) => (
                <motion.button
                  key={activity}
                  whileHover={{ scale: 1.05 }}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {ACTIVITIES.filter(
                (a) => a !== formData.primaryActivity && a !== formData.secondaryActivity
              ).map((activity) => (
                <motion.button
                  key={activity}
                  whileHover={{ scale: 1.05 }}
      ...
      >
        <div className="space-y-4">
          ...
          {leadersError && (
            <div className="flex items-center justify-between gap-3 border border-[rgba(var(--mg-danger),0.4)] rounded p-3 bg-[rgba(var(--mg-danger),0.1)]">
              <span className="text-[rgba(var(--mg-danger),0.9)] text-sm">{leadersError}</span>
              <MobiGlasButton
              ...
              {formData.leaders.map((leader, index) => (
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                    <div>
                      <label
                        htmlFor={`mission-leader-role-${index}`}
                        className="mg-subtitle block mb-1 text-xs"
                      >
                        ROLE
                      </label>
                      <select
                    ...
                    <div>
                      <label
                        htmlFor={`mission-leader-user-${index}`}
                        className="mg-subtitle block mb-1 text-xs"
                      >
                        LEADER
                      </label>
        ...
        rightContent={
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 text-sm text-[rgba(var(--mg-text),0.7)]">
              <ShipIcon />
              <span>
                Ships: <strong className="text-[rgba(var(--mg-primary),1)]">{totalShips}</strong>
              </span>
            </div>
            <MobiGlasButton
              onClick={addShipRequirement}
              variant="primary"
              size="sm"
              leftIcon={<PlusIcon />}
              withScanline
      ...
      >
        <div className="space-y-4">
              ...
              {formData.shipRequirements.map((ship, index) => (
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label
                        htmlFor={`mission-ship-category-${index}`}
                        className="mg-subtitle block mb-1 text-xs"
                      >
                        CATEGORY
                      </label>
                      <select
                        ...
                        value={ship.category}
                        onChange={(e) =>
                          updateShipRequirement(index, 'category', e.target.value as ShipCategory)
                        }
                        className="mg-input w-full text-sm"
                      >
                    ...
                    <div>
                      <label
                        htmlFor={`mission-ship-count-${index}`}
                        className="mg-subtitle block mb-1 text-xs"
                      >
                        COUNT
                      </label>
                      <div className="flex items-center">
                        <button
                          onClick={() =>
                            updateShipRequirement(index, 'count', Math.max(1, ship.count - 1))
                          }
                        ...
                        <button
                          onClick={() =>
                            updateShipRequirement(index, 'count', Math.min(20, ship.count + 1))
                          }
          ...
          {errors.shipRequirements && (
            <div className="text-[rgba(var(--mg-danger),0.8)] text-sm">
              {errors.shipRequirements}
            </div>
          )}
        </div>
        ...
        rightContent={
          <div className="flex items-center gap-4">
            <div className="text-sm text-[rgba(var(--mg-text),0.7)]">
              Personnel:{' '}
              <strong className="text-[rgba(var(--mg-secondary),1)]">{totalPersonnel}</strong>
            </div>
            <MobiGlasButton
              onClick={addPersonnelRequirement}
              variant="primary"
              size="sm"
              leftIcon={<PlusIcon />}
              withScanline
              ...
              {formData.personnelRequirements.map((personnel, index) => (
                ...
                >
                  ...
                  <div className="space-y-3">
                    <div>
                      <label
                        htmlFor={`mission-personnel-profession-${index}`}
                        className="mg-subtitle block mb-1 text-xs"
                      >
                        PROFESSION
                      </label>
                      <select
                        ...
                        onChange={(e) =>
                          updatePersonnelRequirement(
                            index,
                            'profession',
                            e.target.value as PersonnelProfession
                          )
                        }
                        className="mg-input w-full text-sm"
                      >
                    ...
                    <div>
                      <label
                        htmlFor={`mission-personnel-count-${index}`}
                        className="mg-subtitle block mb-1 text-xs"
                      >
                        COUNT
                      </label>
      ...
      <MobiGlasPanel title="Mission Briefing">
        <div className="space-y-4">
          <div>
            <label htmlFor="mission-objectives" className="mg-subtitle block mb-2">
              OBJECTIVES
            </label>
            <textarea
          ...
          <div>
            <label htmlFor="mission-briefing" className="mg-subtitle block mb-2">
              DETAILED BRIEFING
            </label>
            <textarea
              id="mission-briefing"
          ...
          <div>
            <label htmlFor="mission-equipment" className="mg-subtitle block mb-2">
