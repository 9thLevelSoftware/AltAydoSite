    ...
    try {
      ...
      if (result?.error) {
        ...
        if (result.error.includes('Database connection error')) {
          setAuthError(
            'Database connection failed. The system is currently unavailable. Please try again later or contact support.'
          );
        } else {
        ...
        try {
          ...
          setIsLoading(false);
          // Validate the returned URL to prevent open redirect
          router.push(isValidCallbackUrl(result.url));
        } catch {
    ...
    >
      ...
      >
        <div className="mg-panel bg-[rgba(var(--mg-panel-dark),0.8)] backdrop-blur-md p-6 rounded-sm relative">
          ...
          <MobiGlasFormError message={authError || ''} className="mb-4" />
          ...
          <div className="my-6 flex items-center">
            ...
            <span className="px-3 text-xs text-[rgba(var(--mg-text),0.6)] font-quantify tracking-wider">
              OR
            </span>
            <div className="flex-1 h-px bg-[rgba(var(--mg-primary),0.2)]"></div>
          </div>
          ...
          <MobiGlasButton
            ...
            className="mb-4 bg-[rgba(88,101,242,0.1)] border-[rgba(88,101,242,0.3)] hover:bg-[rgba(88,101,242,0.2)] text-white"
            leftIcon={
