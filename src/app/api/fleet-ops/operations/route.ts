...
const createOperationSchema = z.object({
  ...
  // Creation may only start an operation in an initial state; later lifecycle
  // changes are driven through the status-transition logic in the PUT handler.
  status: z.enum(['Planning', 'Briefing']),
  plannedDateTime: z
    .string()
  ...
  diagramLinks: z.array(httpUrlSchema).optional().default([]),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const isLeadership =
      auth.role === 'Director' ||
      auth.role === 'Manager' ||
      auth.role === 'Board Member' ||
      auth.clearanceLevel >= 3;
    ...
    logger.error(
      ...
      { route: '/api/fleet-ops/operations' }
    );
